...
# NOTE: This is the INTENTIONALLY-BAD variant of video_caption.py.
# All of the audio-description quality guidance has been deliberately removed:
# no accessibility guidelines, no character-identification rules, no
# anti-hallucination instructions, and no genre-specific strategies. Only the
# structural scaffolding needed to keep the output parseable (timestamp format
# + JSON shape) is kept, so the downstream pipeline still runs.
AUDIO_DESCRIPTION_GUIDELINES = ""


def get_genre_strategy(video_category: str) -> str:
    """Genre strategies removed in the bad variant; always returns ''."""
    return ""


def get_genre_label(video_category: str) -> str:
    """No genre dispatch in the bad variant."""
    return "none"
...
PROMPT_TEMPLATE = """
        ...
        Watch this video scene and describe everything you notice.
        {voice_rule}
        ============================================================
        FORMAT REQUIREMENTS
        ============================================================
        ...
        - Each event has a "type" of either "Visual" or "Text on Screen".
...
MODEL_CONFIGS = {
    MODEL_GEMINI: {
        ...
        "system_instruction": "You are an AI that watches videos and describes them.",
        "max_retries": 2,
        "video_fps": 4.5,
    },
    MODEL_QWEN: {
